...
const State = () => {
  ...
  useEffect(() => {
    const allStates = async () => {
      try {
        ...
        setTotalPages(Math.ceil(response.data.states.length / itemsPerPage));
      } catch (error) {
  ...
  }, [searchTerm, states, itemsPerPage]);

  // Get current states for pagination
  ...
  const handleAddState = async () => {
    ...
    try {
      ...
      if(response.data.status === 'success') {
        ...
        setFilteredStates([...filteredStates, response.data.state]);
    ...
    try {
      ...
      if(response.data.status === 'success') {
        setStates(states.map(state =>
          state._id === currentState._id ? { ...state, name: currentState.name } : state
        ));
        setFilteredStates(filteredStates.map(state =>
          state._id === currentState._id ? { ...state, name: currentState.name } : state
        ));
  ...
  const handleDeleteState = async () => {
    try {
      ...
      if(response.data.status === 'success') {
        ...
        setFilteredStates(filteredStates.filter(state => state._id !== stateToDelete));
  ...
  const exportToCSV = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      + "ID,Name,Created At\n"
      + states.map(state => `${state._id},${state.name},${new Date(state.createdAt).toLocaleDateString()}`).join("\n");
      ...
      <div className="max-w-6xl mx-auto">
                  ...
                  <tr>
                    <td colSpan="4" className="px-6 py-8 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-500">
                        ...
                        <p className="text-sm">Try adding a new state or adjusting your search</p>
                      </div>
                    </td>
                  </tr>
                )}
                ...
                <div>
                  <p className="text-sm text-gray-700">
                    ...
                    <span className="font-medium">
                      {indexOfLastItem > filteredStates.length ? filteredStates.length : indexOfLastItem}
                    </span>{' '}
                    of <span className="font-medium">{filteredStates.length}</span> results
                  </p>
                </div>
                <div>
          ...
          <div className="fixed inset-0  bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white border border-gray-300 rounded-lg shadow-xl max-w-md w-full p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-800">Add New State</h3>
        ...
        {isEditModalOpen && (
          <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white border border-gray-300 rounded-lg shadow-xl max-w-md w-full p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-800">Edit State</h3>
        ...
        {isDeleteModalOpen && (
          <div className="fixed inset-0  bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white border border-gray-300 rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <div className="mb-4">
                <p className="text-gray-600">Are you sure you want to delete this state? This action cannot be undone.</p>
              </div>
